...
namespace Injections
{
    public static class InjectorExtensions
    {
        ...
        public static void ToFactory(this IInjector injector, Type @interface, Type impl)
        {
            CheckImpl(@interface, impl);
            injector.Register(@interface, new FactoryResolver(impl));
        }
        ...
        public static void ToSingleton<T>(this IInjector injector, Func<T> factory)
        {
            CheckImpl<T>();
            injector.Register(typeof(T), new DynamicSingletonResolver<T>(factory));
        }

        public static void ToSingleton<TApi, TImpl>(this IInjector injector, bool oneInstance = true,
            bool registerImpl = false) where TImpl : TApi
        {
            CheckImpl<TImpl>();
        ...
        public static void ToSingleton(this IInjector injector, Type @interface, Type impl, bool oneInstance = true)
        {
            CheckImpl(@interface, impl);
            injector.Register(@interface, new SingletonResolver(impl, oneInstance));
        }
        ...
        private static void CheckImpl(Type @interface, Type impl)
        {
            ...
            if (!@interface.IsAssignableFrom(impl))
            {
                throw new ArgumentException();
            }
        }
